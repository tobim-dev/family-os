...
async function loadMeals(){if(mealLoading)return;mealLoading=true;try{mealState=await api('/meals?start='+mealStart);if(view==='meals'&&state)render();}catch(e){toast(e.message);}finally{mealLoading=false;}}
function mealsHTML(){
  ...
  const duplicateIds=s?.duplicate_ids||{},recipeEditsBlocked=Boolean(duplicateIds.ingredients?.length||duplicateIds.shopping_recipes?.length);
  const ambiguous=i=>(duplicateIds[i.group==='ingredient'?'ingredients':'additional']||[]).includes(i.id);
  const ambiguityNotice=Object.values(duplicateIds).some(ids=>ids.length)?'<div class="note">Cookidoo liefert einige Kennungen mehrfach. Alle Positionen und Mengen bleiben sichtbar. Markierte Artikel bitte direkt in Cookidoo abhaken; Änderungen an Rezeptzutaten sind bei mehrdeutigen Rezept- oder Zutatenkennungen vorerst nur dort möglich.</div>':'';
